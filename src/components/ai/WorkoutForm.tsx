...
export default function WorkoutForm() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
        await savePlanHistory('workout', result);
        setSaved(true);
    } catch (e) {
        console.error(e);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {result && (
                <div className="space-y-2">
                    ...
                    <textarea name="feedback" className="w-full p-2 bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-sm" placeholder="e.g. Make it more intense, add more cardio, or focus on leg day..."></textarea>
                </div>
            )}
